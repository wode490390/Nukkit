...
public class BlockItemFrame extends BlockTransparentMeta {
    ...
    public boolean onActivate(Item item, Player player) {
        ...
        if (itemFrame.getItem().getId() == Item.AIR) {
            ...
            this.getLevel().addSound(this, SoundEnum.BLOCK_ITEMFRAME_ADD_ITEM);
        } else {
            itemFrame.setItemRotation((itemFrame.getItemRotation() + 1) % 8);
            this.getLevel().addSound(this, SoundEnum.BLOCK_ITEMFRAME_ROTATE_ITEM);
        }
        return true;
    }
    ...
    public boolean place(Item item, Block block, Block target, BlockFace face, double fx, double fy, double fz, Player player) {
        if (!target.isTransparent() && face.getIndex() > 1 && !block.isSolid()) {
            ...
            this.getLevel().addSound(this, SoundEnum.BLOCK_ITEMFRAME_PLACE);
            return true;
        }
        return false;
    }
