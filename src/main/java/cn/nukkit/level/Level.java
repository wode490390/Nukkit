...
//    private final List<BlockUpdateEntry> nextTickUpdates = Lists.newArrayList();
    ...
    public Level(Server server, String name, String path, Class<? extends LevelProvider> provider) {
        ...
        if (convert) {
            throw new LevelException("We don't need to load LevelDB maps!");
        }
        try {
            if (convert) {
        ...
        if (convert) {
            log.info(this.server.getLanguage().translateString("nukkit.level.updating", TextFormat.GREEN + this.provider.getName() + TextFormat.WHITE));
        ...
        log.info(this.server.getLanguage().translateString("nukkit.level.preparing", TextFormat.GREEN + this.provider.getName() + TextFormat.WHITE));
        ...
        for (Vector3 b : blocks) {
            ...
            try {
                ...
                try {
                    ...
                    log.throwing(new IllegalStateException("Unable to create BlockUpdatePacket at (" + x + ", " + y + ", " + z + ") in " + this.getName(), e));
                    continue;
                }
            }
        ...
        if (!this.loaders.isEmpty()) {
            for (ChunkLoader loader : this.loaders.values()) {
                ...
                for (int chunk = 0; chunk < chunksPerLoader; ++chunk) {
                    int dx = ThreadLocalRandom.current().nextInt(2 * randRange) - randRange;
                    int dz = ThreadLocalRandom.current().nextInt(2 * randRange) - randRange;
    ...
    public boolean save(boolean force) {
        ...
        this.provider.setTime((long) this.time);
    ...
    private void computeRemoveBlockLight(int x, int y, int z, int currentLight, Queue<Object[]> queue, Queue<Vector3> spreadQueue, Map<BlockVector3, Boolean> visited, Map<BlockVector3, Boolean> spreadVisited) {
    ...
    private void computeSpreadBlockLight(int x, int y, int z, int currentLight, Queue<Vector3> queue, Map<BlockVector3, Boolean> visited) {
        int current = this.getBlockLightAt(x, y, z);
        BlockVector3 index = new BlockVector3(x, y, z);
...
//        }
    ...
    public Item useBreakOn(Vector3 vector, BlockFace face, Item item, Player player, boolean createParticles) {
        ...
        Item[] drops;
        if (item == null) {
            item = new ItemBlock(Block.get(Block.AIR), 0, 0);
        }
    ...
    public void dropExpOrb(Vector3 source, int exp, Vector3 motion, int delay) {
        motion = (motion == null) ? new Vector3(ThreadLocalRandom.current().nextDouble() * 0.2 - 0.1, 0.2, ThreadLocalRandom.current().nextDouble() * 0.2 - 0.1) : motion;
        CompoundTag nbt = new CompoundTag()
    ...
    private final static Entity[] EMPTY_ENTITY_ARR = new Entity[0];
    private final static Entity[] ENTITY_BUFFER = new Entity[512];
    ...
    public synchronized boolean unloadChunk(int x, int z, boolean safe, boolean trySave) {
        ...
        try {
            ...
            log.error(this.server.getLanguage().translateString("nukkit.level.chunkUnloadError", e.toString()));
            log.throwing(e);
        }
