...
import cn.nukkit.level.generator.biomegrid.MapLayerWhittaker.ClimateType;
import cn.nukkit.level.generator.biomegrid.MapLayerZoom.ZoomType;
...
public abstract class MapLayer {
    ...
    public static MapLayer[] initialize(long seed, int environment, int worldType) {
        if (environment == DimensionIds.OVERWORLD && worldType == Generator.TYPE_FLAT) {
            return new MapLayer[]{new MapLayerBiomeConstant(seed, EnumBiome.PLAINS.id), null};
        } else if (environment == DimensionIds.NETHER) {
            return new MapLayer[]{new MapLayerBiomeConstant(seed, EnumBiome.HELL.id), null};
        } else if (environment == DimensionIds.THE_END) {
            return new MapLayer[]{new MapLayerBiomeConstant(seed, EnumBiome.THE_END.id), null};
        }

        int zoom = 2;
        if (worldType == Generator.TYPE_LARGE_BIOMES) {
            zoom = 4;
        }
