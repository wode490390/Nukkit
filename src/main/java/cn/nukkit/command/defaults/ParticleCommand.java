    ...
    public boolean execute(CommandSender sender, String commandLabel, String[] args) {
        ...
        for (int i = 0; i < count; i++) {
            particle.setComponents(
                    position.x + (ThreadLocalRandom.current().nextFloat() * 2 - 1),
                    position.y + (ThreadLocalRandom.current().nextFloat() * 2 - 1),
                    position.z + (ThreadLocalRandom.current().nextFloat() * 2 - 1)
            );
