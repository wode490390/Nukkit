...
public class ItemFireworkCharge extends Item {
    ...
    public boolean onActivate(Level level, Player player, Block block, Block target, BlockFace face, double fx, double fy, double fz) {
        if (block.getId() == AIR && (target instanceof BlockSolid || target instanceof BlockSolidMeta)) {
            ...
            if (target.getId() == OBSIDIAN) {
                ...
                if (sizeX >= 2 && sizeX <= MAX_PORTAL_SIZE) {
                    ...
                    for (int height = 0; height < innerHeight; height++)    {
                        for (int width = 0; width < innerWidth; width++)    {
                            level.setBlock(new Vector3(scanX - width, scanY + height, scanZ), Block.get(NETHER_PORTAL));
                        }
                    }
                    ...
                    for (int height = 0; height < innerHeight; height++)    {
                        for (int width = 0; width < innerWidth; width++)    {
                            level.setBlock(new Vector3(scanX, scanY + height, scanZ - width), Block.get(NETHER_PORTAL));
                        }
                    }
            ...
            if ((player.gamemode & 0x1) == 0 && this.useOn(block)) {
